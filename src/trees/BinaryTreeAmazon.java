...
public class BinaryTreeAmazon extends BinarySearchTree {
	...
	public void findLeastCommonAncestor(Comparable data1,Comparable data2)
	{
		...
		Tnode lca=findLCAUtil(root,data1,data2,v1,v2);
		if(lca!=null){
	...
	private Tnode findLCAUtil(Tnode node,Comparable data1,Comparable data2,boolean[]v1,boolean[] v2)
	{
		if(node==null)
			return null;
		Tnode lca_left,lca_right;
	...
	private boolean findinTree(Tnode node,Comparable data){
	...
	private void getSumGreaterUtil(Tnode node,int[] sumGreater){
		if(node==null)
			return ;
	...
	private int getMaxSumLeaftoLeafUtil(Tnode node,int[] maxSum){
		if(node==null)
			return 0;
